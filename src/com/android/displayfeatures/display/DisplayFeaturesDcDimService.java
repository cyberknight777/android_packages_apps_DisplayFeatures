...
public class DisplayFeaturesDcDimService extends Service {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_SCREEN_OFF)) {
                ...
                if (enabled) FileUtils.writeLine(mConfig.getDcDimPath(), "0");
                Intent dcDimIntent = new Intent(context,
                com.android.displayfeatures.display.DisplayFeaturesDcDimService.class);

                if (enabled) context.startService(dcDimIntent);
                else context.stopService(dcDimIntent);

                Intent mChangedIntent = new Intent(mConfig.ACTION_DC_DIM_SERVICE_CHANGED);

                mChangedIntent.putExtra(mConfig.EXTRA_DC_DIM_STATE, enabled);
